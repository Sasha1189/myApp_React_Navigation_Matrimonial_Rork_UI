...
import { sqlProfileTable } from "../db/schema";
...
const handlePaidBulkSync = async (
  ...
  const isCompleted = storage.getBoolean(
  ...
  const now = Date.now();
  storage.set(`is_initial_sync_done_${targetCollection}`, true);
  storage.set("last_synced_at", maxTimestamp || now);
  storage.set(`last_delta_run_${targetCollection}`, now);
...
export const performDeltaSync = async (
  ...
  const now = Date.now();
  const lastRunTime =
    storage.getNumber(`last_delta_run_${targetCollection}`) || 0;
  ...
  storage.set("last_synced_at", latestTimestamp);
